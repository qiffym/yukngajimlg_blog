...
export function CommandPalette({ open, setOpen }) {
    ...
    const debouncedSearch = useCallback(
        debounce(async (value) => {
            const { data } = await axios(route('articles.search', { search: value }));
            setResults(data);
        }, 500),
        [],
    );
            ...
            <CommandList className="p-2">
                ...
                {results.length > 0 ? (
                    <CommandGroup heading="Results">
                        {results.map((article) => (
                            <CommandItem key={article.id} className={cmdic} onSelect={() => router.get(article.href)}>
                                <IconSketchbookFill />
                                {article.title}
                            </CommandItem>
                        ))}
                    ...
                    <CommandGroup heading="Suggestions">
                        ...
                        <CommandItem className={cmdic} onSelect={() => router.get(route('internal-articles.index'))}>
                            <IconNotes />
                            List articles
                        </CommandItem>
